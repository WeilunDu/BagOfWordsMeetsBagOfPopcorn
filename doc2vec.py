...
'''
README: this modified module is aimed to achieve the following goals: 

1, provides an online training method to produce a document vector for unseen review

2, The original open source implementation of doc2vec violates the contract that in 
heirachical softmax(the output layer), the Huffman tree should be built on top of
words instead of a mixture of words and labels. 

The new design will reduce the max depth of the huffman tree by only using word vector. 
But it is achieved at the cost of a new contract with user such that all the document labels 
should have it first letter in uppercase. 


Please diff against the original open source 
implementation for doc2vec.py at 
...
'''
I modified doc2vec_inner because the original implementation uses the 
length of the document vector's Huffman code to test for whether a document
label exists or not. But the design contract here is that every document must
have at least one label. Thus, I don't think the modification will change any results.  
'''
try:
...
class Doc2Vec(Word2Vec):
    ...
    '''
    train a single document vector online, most of the code are adapted from the 
    existing algorithm but to only simplify certain conditions. 
    In backprop, we only update doc vector by fixing heirachical softmax weights and 
    word vectors. The quality of the trained doc vector depends on the number of words 
    in sentence that appears in previous trained model and the number of epoch
    '''
    ...
    '''
    Here comes the hack to not build Huffman tree with document labels:
    When building a huffman tree, we need to first feed all the words into a priority queue,
    so I filter out all the words that starts with uppercase because in the _vocab_from function,
    I turn all words into lowercase. 
    The contract with the user is that all labels must starts with upper case
    '''
    ...
    def _vocab_from(sentences):
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            for label in sentence.labels:
                ...
                if label[0].islower():
                    import warnings
                    warnings.warn("label must have its first letter to be uppercase")
                    raise RuntimeError("label must have its first letter to be uppercase")
                if label in vocab:
                    vocab[label].count += sentence_length
                else:
                    vocab[label] = Vocab(count=sentence_length)
            for word in sentence.words:
                #a key difference here is to turn all words into lowercase
                word = word.lower()
                total_words += 1
                if word in vocab:
                    vocab[word].count += 1
